...
const pickUpIcon = new L.DivIcon({
  ...
  <div style="background:#0a0a0a;color:#fff;padding:5px 13px;border-radius:100px;
  font-size:10px;font-weight:800;letter-spacing:0.1em;text-transform:uppercase;white-space:nowrap;
  font-family:system-ui">
  PICKUP
  </div>
  <div style="width:2px;height:9px;background:#0a0a0a;">
  </div>
  <div style="width:10px;height:10px;background:#0a0a0a;border-radius:50%;border:2.5px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.3)">
  </div>`,
});
...
const driverIcon = new L.DivIcon({
  ...
  <path d="M5 11L6.5 6.5H17.5L19 11" stroke="#fff" stroke-width="2" stroke-linecap="round"/>
  <rect x="3" y="11" width="18" height="7" rx="2" stroke="#fff" stroke-width="2" />
  <circle cx="7.5" cy="19" r="2" fill="#fff"/>
  <circle cx="16.5" cy="19" r="2" fill="#fff"/>
...
function LiveRideMap({
  ...
  useEffect(() => {
    ...
    fetchRoute();
  }, [driverLocation, dropLocation, mapStatus, pickUpLocation]);

  const showPickUpMarker = mapStatus === "arriving";
  const showPickUpRoute =
    mapStatus === "arriving" && (routeToPickUp?.length ?? 0) > 0;
  const showDropRoute =
    mapStatus != "completed" && (routeToDrop?.length ?? 0) > 0;
  return (
