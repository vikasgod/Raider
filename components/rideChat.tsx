...
function RideChat({ currentRole, bookingId, userName, driverName }: any) {
  ...
  const formateTime = (dateInput: Date | string) => {
...
border-zinc-100"
      ...
      <div
        className="flex-1 overflow-y-auto px-4 py-4 space-y-3 ng-zinc-50"
      ...
      >
        ...
        {messages.length > 0 &&
          messages.map((v, i) => {
            const isMine = v.sender.toString() === currentRole;
            return (
              <motion.div
                key={i}
      ...
      <AnimatePresence>
        {showAI && messages.length > 0 && (
          <motion.div
            ...
            className="flex-shrink-0 overflow-hidden border-t border-zinc-100 
                bg-white"
          >
                ...
                <div className="flex flex-col gap-1.5">
                  {suggestions.map((v, i) => (
                    <motion.div
                      ...
                      className="text-left text-sm text-zinc-700 bg-zinc-50 
    hover:bg-violet-500 hover:text-violet-700 border border-zinc-100
    hover:border-violet-200 rounded-xl
    px-3 py-2 transition-all"
                  ...
                  hover:text-violet-700 font-semibold
      ...
      <div className="flex-shrink-0 px-4 pb-4 pt-2 bg-white">
        <div
          className="flex items-center gap-2 bg-zinc-100 rounded-2xl
            pl-3 pr-1.5 py-1.5"
        >
          {messages.length > 0 && (
            <motion.div
